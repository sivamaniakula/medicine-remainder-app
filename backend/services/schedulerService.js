...
const { dispatchReminder } = require("./reminderDispatcher");
...
//
// NEW in Bundle B: right after a DoseLog is created, we call
// dispatchReminder() to actually send the reminder via the patient's
// preferred channel (push/WhatsApp/voice). This is fire-and-forget from
// the scheduler's point of view - dispatchReminder handles its own errors
// so one failed reminder never stops the loop for other patients.
  ...
  try {
    ...
    for (const medication of activeMedications) {
      ...
      try {
        ...
        // Fire the actual reminder (push/WhatsApp/voice) now that the
        // dose log exists. Not awaited with a try/catch here because
        // dispatchReminder already catches its own errors internally.
        dispatchReminder(doseLog);
      } catch (err) {
...
// For voice-channel patients, Twilio's own retry (in the IVR flow) already
// tries once more before this window closes - see BUNDLE_B_NOTES.md.
